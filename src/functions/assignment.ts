...
export async function handleTambahTugas(
	...
	if (!content) {
		const errorResponse =
			'Format salah! Gunakan: /tugas [nama tugas], [deskripsi], [deadline]\n\nContoh: /tugas Data Mining, dikumpulkan ke ethol, 7/7';
...
export async function handleHelp(baseUrl: string, session: string, apiKey: string, chatId: string, reply_to: string) {
...
async function sendMessage(baseUrl: string, session: string, apiKey: string, chatId: string, reply_to: string, text: string) {
